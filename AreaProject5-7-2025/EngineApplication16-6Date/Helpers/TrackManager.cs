...
namespace EngineApplication16_6Date.Helpers
{
    public class TrackManager
    {
        ...
        public TrackManager()
        {
            ...
            localTracks = GetAllTracks(); // مبدأياً جلب التراكات من الداتابيز
        }
                    ...
                    using (var command = new MySqlCommand(query, connection))
                    {
                        using (var reader = command.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                ...
                                // التحقق مما إذا كان هناك بيانات في trackTransaction قبل الإضافة
        ...
        public bool UpdateTrackTransaction(int trackId, double newSpeed, double newAltitude)
        {
            var trackValue = localTracks.FirstOrDefault(t => t.Id == trackId);
            var lastTrackTransactions = trackValue.trackTransactions.LastOrDefault(t => t.TrackId == trackId);

            if (lastTrackTransactions != null)
            {
                lastTrackTransactions.trackspeed = newSpeed;
                lastTrackTransactions.altitude = newAltitude;
                return true;
            }
            return false;
        }
        ...
        public void updateLocationOfTrack1FromTracks()
        {
            localTrackUpdateTimer.Change(0, 1000);
